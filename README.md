...
| Tab         | 內容                                                                 |
| ----------- | -------------------------------------------------------------------- |
| 🗺 導航     | 路徑設定 (A/B/C)、速度/路線模式、瞬移、抓取路徑、開始/停止種花、繞圈 |
...
> **提示：** 首次執行 `python app.py` 時，程式會自動檢查並安裝缺少的基本套件（requests、tkintermapview、Pillow）。pymobiledevice3 及其相依套件仍需手動安裝。
